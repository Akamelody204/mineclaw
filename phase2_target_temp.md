...
## Phase 2.6: 流式/推送模式（长时间运行任务支持）

**目标**: 支持长时间运行的自动化任务（如 Playwright 搜索数据），提供实时进度反馈。

### 设计概述

**两种模式对比：**

| 特性 | 模式 A（当前） | 模式 B（流式/推送） |
|------|---------------|-------------------|
| 请求-响应 | 单次请求完成所有步骤 | 多次请求/推送 |
| 进度反馈 | 无（请求挂起） | 实时推送 |
| 超时风险 | 高（总时间可能超过 API 超时） | 低（分步执行） |
| 适用场景 | 快速任务 | 长时间任务（Playwright 等） |
...
```bash
# 重复调用直到完成
curl http://127.0.0.1:18789/api/sessions/{session-id}/status
```
...
{
  "status": "completed",
  "final_response": "2+3 的结果是 5。现在我来计算第三个加法 3+4。"
}
```

#### 方案 2：Server-Sent Events (SSE) 模式（更优雅）
...
**实时推送：**
```
data: {"type": "message", "role": "Assistant", "content": "好的，我现在帮你计算第一个加法 1 + 2。"}

data: {"type": "tool_call", "tool": "add", "arguments": {"a": 1, "b": 2}}

data: {"type": "tool_result", "content": "3"}

data: {"type": "message", "role": "Assistant", "content": "1+2 的结果是 3。现在我来计算第二个加法 2+3。"}

data: {"type": "completed", "final_response": "2+3 的结果是 5。现在我来计算第三个加法 3+4。"}
```

### 任务清单
- [ ] 设计任务状态数据结构（pending/in_progress/completed/failed）
- [ ] 实现后台任务队列（tokio::spawn）
- [ ] 实现轮询 API：`GET /api/sessions/:id/status`
- [ ] 实现 SSE 流式 API：`GET /api/sessions/:id/stream`（可选）
- [ ] 修改 `send_message` handler 支持 `stream: true` 参数
- [ ] 任务超时和取消机制
