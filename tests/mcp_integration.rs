...
async fn test_mcp_server_integration() {
    ...
    if result.is_ok() {
        ...
        // 检查服务器状态
        let server = manager.get_server("test-server").expect("Server should exist");
        ...
        manager.stop_server("test-server").await.expect("Failed to stop server");
...
async fn test_mcp_tool_call_integration() {
    ...
    if let Ok(_) = manager.start_server(&server_config).await {
        ...
        let echo_result = manager.call_tool(
            "test-server",
            "echo",
        ...
        let add_result = manager.call_tool(
            "test-server",
            "add",
            json!({"a": 40, "b": 2})
        ).await;
        ...
        let error_result = manager.call_tool(
            "test-server",
            "echo",
            json!({})
        ).await;
