...
use axum::serve;
use mineclaw::{create_router, create_provider, Config, AppState, SessionRepository, ToolCoordinator};
...
async fn main() -> mineclaw::Result<()> {
    ...
    if let Some(mcp_config) = &config.mcp {
        if mcp_config.enabled {
            info!("MCP is enabled, starting {} servers", mcp_config.servers.len());
    ...
    let app_state = AppState::new(session_repo, llm_provider, mcp_server_manager_arc, tool_executor, tool_coordinator);
